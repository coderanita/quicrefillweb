...
import { FaTimes, FaUpload, FaPlus, FaExclamationTriangle } from 'react-icons/fa';
import { showSweetAlert } from '../Includes/SweetAlert2';

const AccessoryModal = ({ isOpen, onClose, onCreate }) => {
  ...
  const validate = () => {
    ...
    if (!form.stock.trim()) newErrors.stock = 'Stock count is required';
    return newErrors;
  };
  ...
  const handleCreate = () => {
    ...
    showSweetAlert('Accessory created successfully!', 'success');
  ...
  const handleCancel = () => {
    onClose();
    showSweetAlert('Accessory creation cancelled.', 'info');
  };
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 z-50 flex items-center justify-center">
      <div className="bg-white rounded-lg shadow-md w-full max-w-xl p-6 relative overflow-y-auto max-h-[90vh]">
        ...
        <h2 className="text-xl font-semibold mb-6">Add Accessory</h2>
